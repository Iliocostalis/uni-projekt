...
public:
		  ...
		  [[maybe_unused]] ControlSerializer *cs = nullptr)
	{
		...
		std::vector<uint8_t> lineLength;
		std::tie(lineLength, std::ignore) =
			IPADataSerializer<uint32_t>::serialize(data.lineLength);
		retData.insert(retData.end(), lineLength.begin(), lineLength.end());
		    ...
		    [[maybe_unused]] ControlSerializer *cs = nullptr)
	{
		...
		if (dataSize < 4) {
			LOG(IPADataSerializer, Error)
				<< "Failed to deserialize " << "lineLength"
				<< ": not enough data, expected "
				<< (4) << ", got " << (dataSize);
			return ret;
		}
		ret.lineLength = IPADataSerializer<uint32_t>::deserialize(m, m + 4);
		m += 4;
		dataSize -= 4;
